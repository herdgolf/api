...
func GetScoreCardHandler(c echo.Context) error {
	...
	if res := db.Preload(clause.Associations).Find(&sc, id); res.Error != nil {
		// if res := db.Model(&models.ScoreCard{}).Preload("HolesScores").Find(&sc, id); res.Error != nil {
...
func GetScoreCardsHandler(c echo.Context) error {
	database := db.DB()

	var sc []*models.ScoreCard

	database.Scopes(db.Paginate(c)).Model(&models.ScoreCard{}).Preload("Holes").Find(&sc)
	// database.Scopes(db.Paginate(c)).Find(&sc)
